...
import com.terdfhdfu.wefgvzdhtru.xinajinwidget.SpanTextViewXianJinFenQi;
...
import com.terdfhdfu.wefgvzdhtru.presentxianjin.XianJinFenQiLoginPresent;

import okhttp3.OkHttpClient;
import okhttp3.Request;
import okhttp3.Response;

public class LoginXianJinFenQiActivity extends XActivity<XianJinFenQiLoginPresent> {
    ...
    SpanTextViewXianJinFenQi loginRemindTv;
...
//        c.drawColor(0x00ffffff);
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        isNeedChecked = XianJinFenQiSharedPreferencesUtilis.getBoolFromPref("is_agree_check");
        isNeedVerification = XianJinFenQiSharedPreferencesUtilis.getBoolFromPref("is_code_register");
...
//                "API_BASE_URL_1 = " + Api.API_BASE_URL);
        ...
        loginRemindTv.setText(createSpanTexts(), position -> {
            if (position == 1) {
                ...
                Router.newIntent(LoginXianJinFenQiActivity.this)
    ...
    private void initListener() {
        loginBtn.setOnClickListener(v -> {
            ...
            if (TextUtils.isEmpty(phoneStr)) {
                XianJinFenQiToastUtil.showShort("请输入手机号");
                return;
            }
            if (!StaticXianJinFenQiUtil.isValidPhoneNumber(phoneStr)) {
                XianJinFenQiToastUtil.showShort("请输入正确的手机号");
                return;
            }
            if (TextUtils.isEmpty(verificationStr) && isNeedVerification) {
                XianJinFenQiToastUtil.showShort("验证码不能为空");
                return;
            }
            if (!remindCb.isChecked()) {
                XianJinFenQiToastUtil.showShort("请阅读用户协议及隐私政策");
                return;
            }
            rotateLoading.start();
...
//        c.drawColor(0x00ffffff);
    ...
    public int getLayoutId() {
        return R.layout.activity_xian_jin_login;
    }

    @Override
    public XianJinFenQiLoginPresent newP() {
        return new XianJinFenQiLoginPresent();
    }

    private List<SpanTextViewXianJinFenQi.BaseSpanModel> createSpanTexts() {
        List<SpanTextViewXianJinFenQi.BaseSpanModel> spanModels = new ArrayList<>();
        SpanTextViewXianJinFenQi.ClickSpanModel spanModel = new SpanTextViewXianJinFenQi.ClickSpanModel();
        SpanTextViewXianJinFenQi.TextSpanModel textSpanModel = new SpanTextViewXianJinFenQi.TextSpanModel();
        ...
        spanModel = new SpanTextViewXianJinFenQi.ClickSpanModel();
