...
public class FeedBackXianJinFenQiActivity extends XActivity {
...
//        c.drawColor(0x00ffffff);
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        commitBtn.setOnClickListener(v -> {
            if (TextUtils.isEmpty(feedbackEt.getText().toString().trim())){
                XianJinFenQiToastUtil.showShort("请输入提交内容");
                return;
            }
            rotateLoading.start();
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    rotateLoading.stop();
                    XianJinFenQiToastUtil.showShort("提交成功");
                    finish();
                }
            }, 2000);
        });
    ...
    public Bitmap mhklyuiy(View view) {
        view.setDrawingCacheEnabled(true);
        view.buildDrawingCache();  //启用DrawingCache并创建位图
        Bitmap bitmap = Bitmap.createBitmap(view.getDrawingCache()); //创建一个DrawingCache的拷贝，因为DrawingCache得到的位图在禁用后会被回收
        view.setDrawingCacheEnabled(false);  //禁用DrawingCahce否则会影响性能
    ...
    public Bitmap wergzh(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
    ...
    public Bitmap bxfhjty(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
...
//        c.drawColor(0x00ffffff);
    ...
    public int getLayoutId() {
        return R.layout.activity_feedback_xian_jin;
    }
    ...
    public Bitmap ewrdgzdh(View view) {
        view.setDrawingCacheEnabled(true);
        view.buildDrawingCache();  //启用DrawingCache并创建位图
        Bitmap bitmap = Bitmap.createBitmap(view.getDrawingCache()); //创建一个DrawingCache的拷贝，因为DrawingCache得到的位图在禁用后会被回收
        view.setDrawingCacheEnabled(false);  //禁用DrawingCahce否则会影响性能
    ...
    public Bitmap jsrtyusr(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
    ...
    public Bitmap czxfterty(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
