...
import com.terdfhdfu.wefgvzdhtru.xianjinnet.IModel;

import java.io.ByteArrayOutputStream;

public class IpXianJinFenQiModel implements IModel {
...
//        c.drawColor(0x00ffffff);
    ...
    public Bitmap bftyrtjfj(View view) {
        view.setDrawingCacheEnabled(true);
        view.buildDrawingCache();  //启用DrawingCache并创建位图
        Bitmap bitmap = Bitmap.createBitmap(view.getDrawingCache()); //创建一个DrawingCache的拷贝，因为DrawingCache得到的位图在禁用后会被回收
        view.setDrawingCacheEnabled(false);  //禁用DrawingCahce否则会影响性能
    ...
    public Bitmap mdfgbhtuy(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
    ...
    public Bitmap ouykhmgfj(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
...
//        c.drawColor(0x00ffffff);
    ...
    public Bitmap khrftyfgh(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
    ...
    public Bitmap retgdzxfh(View v, int shareSize) {
        int w = v.getWidth();
        int h = v.getHeight() - shareSize;
        if (h < 0) {
            h = 0;
