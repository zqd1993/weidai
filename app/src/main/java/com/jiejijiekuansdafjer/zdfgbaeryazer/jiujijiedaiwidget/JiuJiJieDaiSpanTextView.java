...
public class JiuJiJieDaiSpanTextView extends androidx.appcompat.widget.AppCompatTextView {

    private List<BaseSpanModel> spanModels;
    private SpanClickListener listener;

    public JiuJiJieDaiSpanTextView(Context context) {
        super(context);
    }

    public JiuJiJieDaiSpanTextView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public JiuJiJieDaiSpanTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private SpannableString getClickableSpan(int position, ClickSpanModel spanModel) {
        ...
        //这一行是设置文字颜色的
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#f75b45")), start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        //这一行主要是用来消除点击文字下划线的
