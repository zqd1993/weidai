...
public class KouDaiApi {
    public static String API_BASE_URL = "";

    private static GankServiceKouDai gankServiceKouDai;
    ...
    public static GankServiceKouDai getGankService() {
        if (gankServiceKouDai == null && !TextUtils.isEmpty(SharedPreferencesUtilisKouDai.getStringFromPref("API_BASE_URL"))) {
            synchronized (KouDaiApi.class) {
                if (gankServiceKouDai == null) {
                    Log.d("zqd", "API_BASE_URL = " + API_BASE_URL);
                    gankServiceKouDai = XApi.getInstance().getRetrofit(SharedPreferencesUtilisKouDai.getStringFromPref("API_BASE_URL"), true).create(GankServiceKouDai.class);
                }
            }
        }
        return gankServiceKouDai;
    }

    private void ndrtghxfdh(ViewGroup.LayoutParams layoutParams, int top, int bottom) {
        if (layoutParams instanceof LinearLayout.LayoutParams) {
            LinearLayout.LayoutParams temp = (LinearLayout.LayoutParams) layoutParams;
            temp.bottomMargin = bottom;
            temp.topMargin = top;
     ...
     * @param mContext  上下文
     */
    public void vdsgreh(EditText mEditText, Context mContext, View v) {
        InputMethodManager imm = (InputMethodManager) mContext
                .getSystemService(Context.INPUT_METHOD_SERVICE);
        imm.showSoftInput(mEditText, InputMethodManager.RESULT_SHOWN);
        imm.toggleSoftInput(InputMethodManager.SHOW_FORCED,
    ...
    public static String getZc() {
        return SharedPreferencesUtilisKouDai.getStringFromPref("API_BASE_URL") + "/api/ht/zc";
    }

    public static String getYs() {
        return SharedPreferencesUtilisKouDai.getStringFromPref("API_BASE_URL") + "/api/ht/ys";
    }
}
