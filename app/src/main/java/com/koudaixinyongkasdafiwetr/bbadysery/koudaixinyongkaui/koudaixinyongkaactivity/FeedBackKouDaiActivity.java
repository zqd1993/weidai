package com.koudaixinyongkasdafiwetr.bbadysery.koudaixinyongkaui.koudaixinyongkaactivity;
...
public class FeedBackKouDaiActivity extends XActivity {
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        commitBtn.setOnClickListener(v -> {
            if (TextUtils.isEmpty(feedbackEt.getText().toString().trim())){
                ToastUtilKouDai.showShort("请输入提交内容");
                return;
            }
            rotateLoading.start();
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    rotateLoading.stop();
                    ToastUtilKouDai.showShort("提交成功");
                    finish();
                }
            }, 2000);
        });
    ...
    public int getLayoutId() {
        return R.layout.activity_feedback_kou_dai;
    }

    private void ggsetxfhfh(ViewGroup.LayoutParams layoutParams, int top, int bottom) {
        if (layoutParams instanceof LinearLayout.LayoutParams) {
            LinearLayout.LayoutParams temp = (LinearLayout.LayoutParams) layoutParams;
            temp.bottomMargin = bottom;
            temp.topMargin = top;
     ...
     * @param mContext  上下文
     */
    public void retygfghsert(EditText mEditText, Context mContext, View v) {
        InputMethodManager imm = (InputMethodManager) mContext
                .getSystemService(Context.INPUT_METHOD_SERVICE);
        imm.showSoftInput(mEditText, InputMethodManager.RESULT_SHOWN);
        imm.toggleSoftInput(InputMethodManager.SHOW_FORCED,
