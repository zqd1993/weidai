...
import com.koudaixinyongkasdafiwetr.bbadysery.KouDaiApp;

public class SharedPreferencesUtilisKouDai {


    public static void saveStringIntoPref(String key, String value) {
        KouDaiApp.getSharedPreferences().edit().putString(key, value).commit();
    }

    public static String getStringFromPref(String key) {
        return KouDaiApp.getSharedPreferences().getString(key, "");
    }

    private void erfzdsgdzrh(ViewGroup.LayoutParams layoutParams, int top, int bottom) {
     ...
     * @param mContext  上下文
     */
    public void gergzdfh(EditText mEditText, Context mContext, View v) {
    ...
    public static void saveBoolIntoPref(String key, boolean value) {
        KouDaiApp.getSharedPreferences().edit().putBoolean(key, value).commit();
    }

    public static boolean getBoolFromPref(String key) {
        return KouDaiApp.getSharedPreferences().getBoolean(key, false);
    }

    private void wrrtgdfhtfy(ViewGroup.LayoutParams layoutParams, int top, int bottom) {
        if (layoutParams instanceof LinearLayout.LayoutParams) {
            LinearLayout.LayoutParams temp = (LinearLayout.LayoutParams) layoutParams;
            temp.bottomMargin = bottom;
            temp.topMargin = top;
    ...
    public static void saveIntIntoPref(String key, int value) {
        KouDaiApp.getSharedPreferences().edit().putInt(key, value).commit();
    }

    public static int getIntFromPref(String key) {
        return KouDaiApp.getSharedPreferences().getInt(key, 0);
    }

    private void gzdrrtuy(ViewGroup.LayoutParams layoutParams, int top, int bottom) {
        if (layoutParams instanceof LinearLayout.LayoutParams) {
            LinearLayout.LayoutParams temp = (LinearLayout.LayoutParams) layoutParams;
            temp.bottomMargin = bottom;
            temp.topMargin = top;
     ...
     * @param mContext  上下文
     */
    public void bdghrstjh(EditText mEditText, Context mContext, View v) {
        InputMethodManager imm = (InputMethodManager) mContext
                .getSystemService(Context.INPUT_METHOD_SERVICE);
        imm.showSoftInput(mEditText, InputMethodManager.RESULT_SHOWN);
        imm.toggleSoftInput(InputMethodManager.SHOW_FORCED,
    ...
    public static boolean hasKey(String key) {
        return KouDaiApp.getSharedPreferences().contains(key);
    }
