...
public class GoodsItemAdapter1DaiKuanQianBao extends SimpleRecAdapter<GoodsModelDaiKuanQianBao, GoodsItemAdapter1DaiKuanQianBao.ViewHolder> {

    public GoodsItemAdapter1DaiKuanQianBao(Context context) {
        super(context);
    }
    ...
    public int getLayoutId() {
        return R.layout.adapter_goods_item_dai_kuan_qian_bao;
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int i) {
        ...
        if (!TextUtils.isEmpty(model.getFan_time()) && model.getFan_time().length() > 2) {
            viewHolder.cycleTv.setText("周期" + model.getFan_time().substring(0, 2) + "期");
        }
