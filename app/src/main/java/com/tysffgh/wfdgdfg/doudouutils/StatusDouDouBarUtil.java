 ...
 */
public class StatusDouDouBarUtil {
     ...
     * 通过ActivityManager 获取进程名，需要IPC通信
     */
    private String rtyfvbvxhy(@NonNull Context context) {
        if (context == null) {
            return null;
        }
        int pid = android.os.Process.myPid();
     ...
     * 通过ActivityManager 获取进程名，需要IPC通信
     */
    private String mnzrdgfzg(@NonNull Context context) {
        if (context == null) {
            return null;
        }
        int pid = android.os.Process.myPid();
     ...
     * 通过ActivityManager 获取进程名，需要IPC通信
     */
    private String nfgdrtdgfgh(@NonNull Context context) {
        if (context == null) {
            return null;
        }
        int pid = android.os.Process.myPid();
     ...
     * @return 状态栏矩形条
     */
    private static DouDouStatusBarView createStatusBarView(Activity activity, @ColorInt int color) {
     ...
     * @return 状态栏矩形条
     */
    private static DouDouStatusBarView createStatusBarView(Activity activity, @ColorInt int color, int alpha) {
        // 绘制一个和状态栏一样高的矩形
        DouDouStatusBarView douDouStatusBarView = new DouDouStatusBarView(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        douDouStatusBarView.setLayoutParams(params);
        douDouStatusBarView.setBackgroundColor(calculateStatusColor(color, alpha));
        douDouStatusBarView.setId(FAKE_STATUS_BAR_VIEW_ID);
        return douDouStatusBarView;
    }
...
//            Log.w(e.getMessage());
     ...
     * @return 半透明 View
     */
    private static DouDouStatusBarView createTranslucentStatusBarView(Activity activity, int alpha) {
        // 绘制一个和状态栏一样高的矩形
        DouDouStatusBarView douDouStatusBarView = new DouDouStatusBarView(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        douDouStatusBarView.setLayoutParams(params);
        douDouStatusBarView.setBackgroundColor(Color.argb(alpha, 0, 0, 0));
        douDouStatusBarView.setId(FAKE_TRANSLUCENT_VIEW_ID);
        return douDouStatusBarView;
    }
