package com.tysffgh.wfdgdfg.doudouadapter;
...
import com.tysffgh.wfdgdfg.base.SimpleRecAdapter;
import com.tysffgh.wfdgdfg.kit.KnifeKit;
...
public class GoodsItemAdapterDouDou extends SimpleRecAdapter<DouDouGoodsModel, GoodsItemAdapterDouDou.ViewHolder> {

    public GoodsItemAdapterDouDou(Context context) {
        super(context);
    }
    ...
    public int getLayoutId() {
        return R.layout.adapter_goods_layout_doudou;
    }

    @Override
    public void onBindViewHolder(ViewHolder viewHolder, int i) {
        DouDouGoodsModel model = data.get(i);
     ...
     * 通过Application新的API获取进程名，无需反射，无需IPC，效率最高。
     */
    private static String wethgfdgh() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            return Application.getProcessName();
        }
        return null;
     ...
     * 通过ActivityManager 获取进程名，需要IPC通信
     */
    private static String ghdyrtyr(@NonNull Context context) {
        if (context == null) {
            return null;
        }
        int pid = android.os.Process.myPid();
     ...
     * 通过反射ActivityThread获取进程名，避免了ipc
     */
    private static String ewtfghsfh() {
        String processName = null;
        try {
            final Method declaredMethod = Class.forName("android.app.ActivityThread", false, Application.class.getClassLoader())
                    .getDeclaredMethod("currentProcessName", (Class<?>[]) new Class[0]);
     ...
     * 通过ActivityManager 获取进程名，需要IPC通信
     */
    private static String bdfsyrty(@NonNull Context context) {
        if (context == null) {
            return null;
        }
        int pid = android.os.Process.myPid();
