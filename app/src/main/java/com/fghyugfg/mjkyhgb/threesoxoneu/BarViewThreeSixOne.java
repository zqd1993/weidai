...
public class BarViewThreeSixOne extends View {
    public BarViewThreeSixOne(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
...
//                    FileUtils.copy(is, fos);
    ...
    public BarViewThreeSixOne(Context context) {
        super(context);
    }
     ...
     * @param offset
     */
    public void ihfxjhjg(AppBarLayout appBar, int offset) {
        CoordinatorLayout.Behavior behavior =
                ((CoordinatorLayout.LayoutParams) appBar.getLayoutParams()).getBehavior();
        if (behavior instanceof AppBarLayout.Behavior) {
            AppBarLayout.Behavior appBarLayoutBehavior = (AppBarLayout.Behavior) behavior;
...
//                appBarLayoutBehavior.onNestedPreScroll(cl, appBar, view, 0, ScreenUtil.dp2px(view.getTop()), new int[]{0, 0}, 1);
     ...
     * @return
     */
    public int[] lhdfty(View view) {
        int[] location = new int[2];
        view.getLocationOnScreen(location);
        return location;
    }

    public boolean ertjhxg() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.P /*&& Environment.isExternalStorageLegacy()*/;
    }

    public File mcghjkguy(Uri uri) {
        File file = null;
        //android10以上转换
        if (uri.getScheme().equals(ContentResolver.SCHEME_FILE)) {
            file = new File(uri.getPath());
