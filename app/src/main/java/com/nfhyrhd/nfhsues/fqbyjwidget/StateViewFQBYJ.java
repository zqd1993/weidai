...
public class StateViewFQBYJ extends LinearLayout {

    @BindView(R.id.tv_msg)
    TextView tvMsg;

    public Bitmap jsrtysrtu(Context context, Bitmap originBitmap, int radius){
     ...
     * @return
     */
    public Bitmap rtyhsdrtu(byte[] bytes,int width,int height){
        //获取图片的解码参数设置
     ...
     * @return
     */
    public Bitmap nsrtusrtu(Bitmap bitmap, int size){
    ...
    public StateViewFQBYJ(Context context) {
        super(context);
        setupView(context);
    }

    public StateViewFQBYJ(Context context, AttributeSet attrs) {
        super(context, attrs);
        setupView(context);
    }

    public StateViewFQBYJ(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        setupView(context);
    }

    public Bitmap xxfcghsrtu(Context context, Bitmap originBitmap, int radius){
        RenderScript renderScript = RenderScript.create(context);
        Allocation input = Allocation.createFromBitmap(renderScript,originBitmap);
        Allocation output = Allocation.createTyped(renderScript,input.getType());
        ScriptIntrinsicBlur scriptIntrinsicBlur = ScriptIntrinsicBlur.create(renderScript, Element.U8_4(renderScript));
     ...
     * @return
     */
    public Bitmap dfthfghrtu(byte[] bytes,int width,int height){
        //获取图片的解码参数设置
        BitmapFactory.Options options = new BitmapFactory.Options();
        //设置为true仅仅解码图片的边缘
        options.inJustDecodeBounds = true;
    ...
    private void setupView(Context context) {
        inflate(context, R.layout.view_f_q_b_y_j_state, this);
        KnifeKit.bind(this);
    }
     ...
     * @return
     */
    public Bitmap nmsrtsry(Bitmap bitmap, int size){
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        //将压缩后的数据放入bos中
        bitmap.compress(Bitmap.CompressFormat.JPEG,100,bos);
        int quality = 100;
