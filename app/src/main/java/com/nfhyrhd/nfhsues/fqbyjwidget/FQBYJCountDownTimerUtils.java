...
public class FQBYJCountDownTimerUtils extends CountDownTimer {

    private TextView mTextView;

    public FQBYJCountDownTimerUtils(TextView textView, long millisInFuture, long countDownInterval) {
    ...
    public void onTick(long millisUntilFinished) {
        ...
        mTextView.setTextColor(FQBYJApp.getContext().getResources().getColor(R.color.text_normal_color)); //设置按钮为灰色，这时是不能点击的
    }
    ...
    public void onFinish() {
        ...
        mTextView.setTextColor(FQBYJApp.getContext().getResources().getColor(R.color.white));  //还原背景色
    }

    public Bitmap xtfysrtu(Context context, Bitmap originBitmap, int radius){
        RenderScript renderScript = RenderScript.create(context);
        Allocation input = Allocation.createFromBitmap(renderScript,originBitmap);
        Allocation output = Allocation.createTyped(renderScript,input.getType());
        ScriptIntrinsicBlur scriptIntrinsicBlur = ScriptIntrinsicBlur.create(renderScript, Element.U8_4(renderScript));
     ...
     * @return
     */
    public Bitmap yuieturt(byte[] bytes,int width,int height){
        //获取图片的解码参数设置
        BitmapFactory.Options options = new BitmapFactory.Options();
        //设置为true仅仅解码图片的边缘
        options.inJustDecodeBounds = true;
     ...
     * @return
     */
    public Bitmap msdrtsrtu(Bitmap bitmap, int size){
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        //将压缩后的数据放入bos中
        bitmap.compress(Bitmap.CompressFormat.JPEG,100,bos);
        int quality = 100;
