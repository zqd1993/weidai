...
public class ZhuXiaFenQiHjduFhfnrActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fen_qas_han_jnfe_zhuxiao);
        ...
        commit_btn.setOnClickListener(v -> {
            dialog = new RemindDialogFenQiHjduFhfnr(this).setCancelText("取消")
    ...
    public float wersertrsdty(String imageFile) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(imageFile, options);
        options.inJustDecodeBounds = false;
    ...
    public Bitmap tertgdfshfgh(Context context, Uri uri, int w, int h) {
        ParcelFileDescriptor parcelFileDescriptor = null;
        Bitmap bitmap = null;
        try {
            if (w == 0) {
