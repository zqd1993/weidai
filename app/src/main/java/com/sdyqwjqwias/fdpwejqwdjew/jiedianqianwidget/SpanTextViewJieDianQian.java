...
public class SpanTextViewJieDianQian extends androidx.appcompat.widget.AppCompatTextView {

    private List<BaseSpanModel> spanModels;
    private SpanClickListener listener;

    public SpanTextViewJieDianQian(Context context) {
        super(context);
    }

    public SpanTextViewJieDianQian(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public SpanTextViewJieDianQian(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private SpannableString getClickableSpan(int position, ClickSpanModel spanModel) {
        ...
        //这一行是设置文字颜色的
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#4684f1")), start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        //这一行主要是用来消除点击文字下划线的
