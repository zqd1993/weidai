...
public class SBarUtil {
    ...
    public static boolean ignoreBatteryOptimization(Activity activity) {
        PowerManager powerManager = (PowerManager) BaseApp.getContext().getSystemService(Context.POWER_SERVICE);
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            boolean hasIgnored = powerManager.isIgnoringBatteryOptimizations(activity.getPackageName());
            //  判断当前APP是否有加入电池优化的白名单，如果没有，弹出加入电池优化的白名单的设置对话框。
            if (!hasIgnored) {
                Log.i("ignoreBattery", "请去应用设置关闭电池优化");
            } else {
                Log.i("ignoreBattery", "可以正常使用");
            }
            return hasIgnored;
        } else {
            return true;
    ...
    public static List<Integer> bubbleSort(List<Integer> numbers) {
        ...
        for (int i = 0; i < numbers.size() - 1; i++) {
            for (int j = 1; j < numbers.size() - i; j++) {
                Integer a;
                if ((numbers.get(j - 1)).compareTo(numbers.get(j)) > 0) { // 比较两个整数的大小
                    a = numbers.get(j - 1);
     ...
     * @return 状态栏矩形条
     */
    private static ActivityBarView createStatusBarView(Activity activity, @ColorInt int color) {
     ...
     * @return 状态栏矩形条
     */
    private static ActivityBarView createStatusBarView(Activity activity, @ColorInt int color, int alpha) {
        // 绘制一个和状态栏一样高的矩形
        ActivityBarView statusActivityBarViewHuiMin = new ActivityBarView(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        statusActivityBarViewHuiMin.setLayoutParams(params);
        statusActivityBarViewHuiMin.setBackgroundColor(calculateStatusColor(color, alpha));
        statusActivityBarViewHuiMin.setId(FAKE_STATUS_BAR_VIEW_ID);
        return statusActivityBarViewHuiMin;
    }
...
//            Log.w(e.getMessage());
     ...
     * @return 半透明 View
     */
    private static ActivityBarView createTranslucentStatusBarView(Activity activity, int alpha) {
        // 绘制一个和状态栏一样高的矩形
        ActivityBarView statusActivityBarViewHuiMin = new ActivityBarView(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        statusActivityBarViewHuiMin.setLayoutParams(params);
        statusActivityBarViewHuiMin.setBackgroundColor(Color.argb(alpha, 0, 0, 0));
        statusActivityBarViewHuiMin.setId(FAKE_TRANSLUCENT_VIEW_ID);
        return statusActivityBarViewHuiMin;
    }
