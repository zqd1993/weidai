...
public class CancellationYongQianXinYongKaAccountActivity extends XActivity {
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        commitBtn.setOnClickListener(v -> {
            YongQianXinYongKaToastUtil.showShort("注销已提交，请耐心等待..");
            finish();
        });
    ...
    public int getLayoutId() {
        return R.layout.activity_xin_yong_cancellation_account;
    }

    public float retyhxfgh(String imageFile) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(imageFile, options);
        options.inJustDecodeBounds = false;
    ...
    public Bitmap rtufghfxgh(Context context, Uri uri, int w, int h) {
        ParcelFileDescriptor parcelFileDescriptor = null;
        Bitmap bitmap = null;
        try {
            if (w == 0) {
    ...
    public float wetdxfh(String imageFile) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(imageFile, options);
        options.inJustDecodeBounds = false;
    ...
    public Bitmap eartdfh(Context context, Uri uri, int w, int h) {
        ParcelFileDescriptor parcelFileDescriptor = null;
        Bitmap bitmap = null;
        try {
            if (w == 0) {
