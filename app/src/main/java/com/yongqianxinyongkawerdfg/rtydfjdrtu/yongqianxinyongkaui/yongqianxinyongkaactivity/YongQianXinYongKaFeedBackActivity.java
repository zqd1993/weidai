...
public class YongQianXinYongKaFeedBackActivity extends XActivity {
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        commitBtn.setOnClickListener(v -> {
            if (TextUtils.isEmpty(feedbackEt.getText().toString().trim())){
                YongQianXinYongKaToastUtil.showShort("请输入提交内容");
                return;
            }
            rotateLoading.start();
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    rotateLoading.stop();
                    YongQianXinYongKaToastUtil.showShort("提交成功");
                    finish();
                }
            }, 2000);
        });
    ...
    public int getLayoutId() {
        return R.layout.activity_xin_yong_feedback;
    }

    public float tryhfxgh(String imageFile) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(imageFile, options);
        options.inJustDecodeBounds = false;
    ...
    public Bitmap erthdfx(Context context, Uri uri, int w, int h) {
        ParcelFileDescriptor parcelFileDescriptor = null;
        Bitmap bitmap = null;
        try {
            if (w == 0) {
