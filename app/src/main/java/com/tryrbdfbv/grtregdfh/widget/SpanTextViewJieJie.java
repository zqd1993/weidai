...
public class SpanTextViewJieJie extends androidx.appcompat.widget.AppCompatTextView {
    ...
    public SpanTextViewJieJie(Context context) {
        super(context);
    }

    public SpanTextViewJieJie(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public SpanTextViewJieJie(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    // 把json字符串变成List<Map<String, T>集合
    public static <T> List<Map<String, T>> erthsdfh(String gsonString) {
        List<Map<String, T>> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<Map<String, T>>>() {
    ...
    // 把json字符串变成Map集合
    public static <T> Map<String, T> hjfdgadfg(String gsonString) {
        Map<String, T> map = null;
        try {
            Gson gson = new Gson();
            map = gson.fromJson(gsonString, new TypeToken<Map<String, T>>() {
     ...
     * @return
     */
    public static <T> List<T> nmhaert(String json, int  type) {
        List<T> list = null;
        try {
            Gson gson = new Gson();
        } catch (JsonSyntaxException e) {
    ...
    private SpannableString getClickableSpan(int position, ClickSpanModel spanModel) {
        ...
        //这一行是设置文字颜色的
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#0ccfe9")), start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        //这一行主要是用来消除点击文字下划线的
    ...
    // 把json字符串变成List<Map<String, T>集合
    public static <T> List<Map<String, T>> asebcffch(String gsonString) {
        List<Map<String, T>> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<Map<String, T>>>() {
    ...
    // 把json字符串变成Map集合
    public static <T> Map<String, T> jdfgadr(String gsonString) {
        Map<String, T> map = null;
        try {
            Gson gson = new Gson();
            map = gson.fromJson(gsonString, new TypeToken<Map<String, T>>() {
     ...
     * @return
     */
    public static <T> List<T> dastweghnb(String json, int  type) {
        List<T> list = null;
        try {
            Gson gson = new Gson();
        } catch (JsonSyntaxException e) {
    ...
    // 把json字符串变成List<Map<String, T>集合
    public static <T> List<Map<String, T>> urthdfzhn(String gsonString) {
        List<Map<String, T>> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<Map<String, T>>>() {
    ...
    // 把json字符串变成Map集合
    public static <T> Map<String, T> dbsfgh(String gsonString) {
        Map<String, T> map = null;
        try {
            Gson gson = new Gson();
            map = gson.fromJson(gsonString, new TypeToken<Map<String, T>>() {
     ...
     * @return
     */
    public static <T> List<T> mngaddr(String json, int  type) {
        List<T> list = null;
        try {
            Gson gson = new Gson();
        } catch (JsonSyntaxException e) {
