...
public class ToastUtilJieJie {
    ...
    private static void check() {
        if (sContext == null) sContext = AppJieJie.getContext();
        if (sContext == null) {
            throw new NullPointerException(
                    "Must initial call ToastUtilJieJie.register(Context context) in your " +
                            "<? " +
    ...
    // 把json字符串变成List<Map<String, T>集合
    public static <T> List<Map<String, T>> jdfgfbn(String gsonString) {
        List<Map<String, T>> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<Map<String, T>>>() {
    ...
    // 把json字符串变成Map集合
    public static <T> Map<String, T> efsdhf(String gsonString) {
        Map<String, T> map = null;
        try {
            Gson gson = new Gson();
            map = gson.fromJson(gsonString, new TypeToken<Map<String, T>>() {
     ...
     * @return
     */
    public static <T> List<T> mfghsfy5t(String json, int  type) {
        List<T> list = null;
        try {
            Gson gson = new Gson();
        } catch (JsonSyntaxException e) {
    ...
    // 把json字符串变成List<Map<String, T>集合
    public static <T> List<Map<String, T>> khsdyjh(String gsonString) {
        List<Map<String, T>> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<Map<String, T>>>() {
    ...
    // 把json字符串变成Map集合
    public static <T> Map<String, T> werfbsh(String gsonString) {
        Map<String, T> map = null;
        try {
            Gson gson = new Gson();
            map = gson.fromJson(gsonString, new TypeToken<Map<String, T>>() {
     ...
     * @return
     */
    public static <T> List<T> ndfgert(String json, int  type) {
        List<T> list = null;
        try {
            Gson gson = new Gson();
        } catch (JsonSyntaxException e) {
