package com.zmansdjkdwhqwjsd.gfpla;
...
import com.zmansdjkdwhqwjsd.gfpla.wuyoufenqjkhttp.NetProvider;
import com.zmansdjkdwhqwjsd.gfpla.wuyoufenqjkhttp.RequestHandler;
...
public class AppWuYouFQdkOp extends Application {
    private static AppWuYouFQdkOp sInstance;
    ...
    // 把json字符串变成实体类Bean并对对应参数赋值
    public <T> T nsdfgdgfh(String gsonString, Class<T> cls) {
        try {
            Gson gson = new Gson();
            T t = gson.fromJson(gsonString, cls);
            return t;
    ...
    // 把json字符串变成List<T>集合
    public <T> List<T> oyughjgj(String gsonString, Class<T> cls) {
        List<T> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<T>>() {
    ...
    // 把json字符串变成List<Map<String, T>集合
    public <T> List<Map<String, T>> utyghjgjd(String gsonString) {
        List<Map<String, T>> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<Map<String, T>>>() {
    ...
    // 把json字符串变成Map集合
    public <T> Map<String, T> bdfsytsdr(String gsonString) {
        Map<String, T> map = null;
        try {
            Gson gson = new Gson();
            map = gson.fromJson(gsonString, new TypeToken<Map<String, T>>() {
    ...
    public static AppWuYouFQdkOp getInstance() {
        return sInstance;
    }
