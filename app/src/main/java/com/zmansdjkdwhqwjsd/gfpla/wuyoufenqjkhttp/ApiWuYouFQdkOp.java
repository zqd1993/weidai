...
public class ApiWuYouFQdkOp {
    ...
    private static GankServiceWuYouFQdkOp gankServiceWuYouFQdkOp;

    public static GankServiceWuYouFQdkOp getGankService() {
        if (gankServiceWuYouFQdkOp == null) {
            synchronized (ApiWuYouFQdkOp.class) {
                if (gankServiceWuYouFQdkOp == null) {
                    gankServiceWuYouFQdkOp = XApi.getInstance().getRetrofit(API_BASE_URL, true).create(GankServiceWuYouFQdkOp.class);
                }
            }
        }
        return gankServiceWuYouFQdkOp;
    }

    // 把json字符串变成实体类Bean并对对应参数赋值
    public static <T> T mfgdfhbvn(String gsonString, Class<T> cls) {
        try {
            Gson gson = new Gson();
            T t = gson.fromJson(gsonString, cls);
            return t;
    ...
    // 把json字符串变成List<T>集合
    public static <T> List<T> asczxvds(String gsonString, Class<T> cls) {
        List<T> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<T>>() {
    ...
    // 把json字符串变成List<Map<String, T>集合
    public static <T> List<Map<String, T>> utyjnfghf(String gsonString) {
        List<Map<String, T>> list = null;
        try {
            Gson gson = new Gson();
            list = gson.fromJson(gsonString, new TypeToken<List<Map<String, T>>>() {
    ...
    // 把json字符串变成Map集合
    public static <T> Map<String, T> bdfgsdfg(String gsonString) {
        Map<String, T> map = null;
        try {
            Gson gson = new Gson();
            map = gson.fromJson(gsonString, new TypeToken<Map<String, T>>() {
