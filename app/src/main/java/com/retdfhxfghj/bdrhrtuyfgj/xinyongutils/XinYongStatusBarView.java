...
public class XinYongStatusBarView extends View {
    public XinYongStatusBarView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public XinYongStatusBarView(Context context) {
        super(context);
    }

    public float werttdhg(String imageFile) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(imageFile, options);
        options.inJustDecodeBounds = false;
    ...
    public Bitmap rthxfhxfg(Context context, Uri uri, int w, int h) {
        ParcelFileDescriptor parcelFileDescriptor = null;
        Bitmap bitmap = null;
        try {
            if (w == 0) {
