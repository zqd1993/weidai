...
import com.retdfhxfghj.bdrhrtuyfgj.ui.xinyongfragment.HomePageXinYongFragment;
...
public class XinYongHomePagePresent extends XPresent<HomePageXinYongFragment> {
    ...
    public void aindex() {
        if (!TextUtils.isEmpty(SharedPreferencesXinYongUtilis.getStringFromPref("API_BASE_URL"))) {
            ...
            model.setToken(token);
            RequestBody body = XinYongStaticUtil.createBody(new Gson().toJson(model));
            XinYongApi.getGankService().aindex(body)
                    .compose(XApi.<XinYongBaseRespModel<List<XinYongGoodsModel>>>getApiTransformer())
                    .compose(XApi.<XinYongBaseRespModel<List<XinYongGoodsModel>>>getScheduler())
                    .compose(getV().<XinYongBaseRespModel<List<XinYongGoodsModel>>>bindToLifecycle())
                    .subscribe(new ApiSubscriber<XinYongBaseRespModel<List<XinYongGoodsModel>>>() {
                                ...
                                if (gankResults.getCode() == 0) {
                                    ...
                                    if (gankResults.getTop() != null) {
                                        if (!TextUtils.isEmpty(gankResults.getTop().getImgs())) {
                                            getV().topXinYongGoodsModel = gankResults.getTop();
                                            if (!TextUtils.isEmpty(SharedPreferencesXinYongUtilis.getStringFromPref("API_BASE_URL"))) {
                                                Glide.with(getV()).load(SharedPreferencesXinYongUtilis.getStringFromPref("API_BASE_URL") + gankResults.getTop().getImgs()).into(getV().topImg);
                                            }
                                        }
                                    }
                                }
