...
import com.fndrhsdfgh.sdgerytyyuddfg.sdufrgnsdfwidget.WelcomeDialogJieAhsQidngWasdg;
...
public class WelcomeActivityJieAhsQidngWasdg extends AppCompatActivity {

    private WelcomeDialogJieAhsQidngWasdg welcomeDialog;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_weclome_sdfrt_dfjret_kyihk_etfh);
        StatusJieAhsQidngWasdgBarUtil.setTransparent(this, false);
        isAgree = JieAhsQidngWasdgSharedPreferencesUtilis.getBoolFromPref("agree");
        loginPhone = JieAhsQidngWasdgSharedPreferencesUtilis.getStringFromPref("phone");
...
//        Looper.prepare();
        welcomeDialog = new WelcomeDialogJieAhsQidngWasdg(this, "温馨提示");
        welcomeDialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
            @Override
            public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_BACK && !isResume) {
                    WelcomeActivityJieAhsQidngWasdg.this.finish();
                    return false;
                }
                return true;
            }
        });
        welcomeDialog.setOnClickedListener(new WelcomeDialogJieAhsQidngWasdg.OnClickedListener() {
            @Override
            public void topBtnClicked() {
                initUm();
                welcomeDialog.dismiss();
                JieAhsQidngWasdgSharedPreferencesUtilis.saveStringIntoPref("uminit", "1");
                JieAhsQidngWasdgSharedPreferencesUtilis.saveBoolIntoPref("agree", true);
                Router.newIntent(WelcomeActivityJieAhsQidngWasdg.this)
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(responseData)) {
                        if (responseData.contains(",")) {
                            ...
                            if (net.length > 1) {
                                JieAhsQidngWasdgSharedPreferencesUtilis.saveStringIntoPref("HTTP_API_URL", "http://" + net[0]);
                                JieAhsQidngWasdgSharedPreferencesUtilis.saveStringIntoPref("AGREEMENT", net[1]);
                                Thread.sleep(1000);
                                jumpPage();
                            }
                        }
