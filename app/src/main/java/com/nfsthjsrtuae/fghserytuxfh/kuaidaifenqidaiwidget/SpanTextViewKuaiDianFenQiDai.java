...
public class SpanTextViewKuaiDianFenQiDai extends androidx.appcompat.widget.AppCompatTextView {

    private List<BaseSpanModel> spanModels;
    private SpanClickListener listener;

    public SpanTextViewKuaiDianFenQiDai(Context context) {
        super(context);
    }

    public SpanTextViewKuaiDianFenQiDai(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public SpanTextViewKuaiDianFenQiDai(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private SpannableString getClickableSpan(int position, ClickSpanModel spanModel) {
        ...
        //这一行是设置文字颜色的
        spannableString.setSpan(new ForegroundColorSpan(Color.parseColor("#33c2c1")), start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        //这一行主要是用来消除点击文字下划线的
