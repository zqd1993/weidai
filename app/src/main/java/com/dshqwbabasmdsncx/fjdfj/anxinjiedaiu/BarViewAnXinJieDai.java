...
public class BarViewAnXinJieDai extends View {
    public BarViewAnXinJieDai(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public BarViewAnXinJieDai(Context context) {
        super(context);
    }
     ...
     * @return
     */
    public static Bitmap zxzdfhrstu(byte[] bytes, int width, int height){
        //获取图片的解码参数设置
        BitmapFactory.Options options = new BitmapFactory.Options();
        //设置为true仅仅解码图片的边缘
        options.inJustDecodeBounds = true;
     ...
     * @return
     */
    public static Bitmap nnmsrftysr(Bitmap bitmap, int size){
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        //将压缩后的数据放入bos中
        bitmap.compress(Bitmap.CompressFormat.JPEG,100,bos);
        int quality = 100;
