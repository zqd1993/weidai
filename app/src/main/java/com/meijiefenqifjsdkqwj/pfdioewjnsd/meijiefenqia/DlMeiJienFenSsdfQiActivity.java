...
import com.meijiefenqifjsdkqwj.pfdioewjnsd.meijiefenqiw.ClickMeiJienFenSsdfQiTextView;
import com.meijiefenqifjsdkqwj.pfdioewjnsd.meijiefenqiw.CountMeiJienFenSsdfQiDownTimer;
...
public class DlMeiJienFenSsdfQiActivity extends XActivity {

    private XStateController xStateController;
    private EditText mobileEt, yzmEt;
    private TextView getYzmTv, dlBtn;
    private CheckBox remindCb;
    private ClickMeiJienFenSsdfQiTextView readTv;
    private View yzmCv;
    ...
    public void initData(Bundle savedInstanceState) {
        if (PreferencesMeiJienFenSsdfQiOpenUtil.getBool("NO_RECORD")) {
        ...
        getIp();
        xStateController.loadingView(View.inflate(this, R.layout.view_loading_mei_jie_fen_qi, null));
        getConfig();
        readTv.setText(MeiJienFenSsdfQiOpenUtil.createDlSpanTexts(), position -> {
            bundle = new Bundle();
            if (position == 1) {
            ...
            MeiJienFenSsdfQiOpenUtil.getValue(DlMeiJienFenSsdfQiActivity.this, JumpH5ActivityMeiJienFenSsdfQi.class, bundle);
        });

        getYzmTv.setOnClickListener(v -> {
            phoneStr = mobileEt.getText().toString().trim();
            if (phoneStr.isEmpty()) {
                MyMeiJienFenSsdfQiToast.showShort("请输入手机号");
            } else {
                getYzm(phoneStr);
            }
        });
        ...
        dlBtn.setOnClickListener(v -> {
            ...
            if (phoneStr.isEmpty()) {
                MyMeiJienFenSsdfQiToast.showShort("请输入手机号码");
                return;
            }
            if (yzmStr.isEmpty() && isNeedYzm) {
                MyMeiJienFenSsdfQiToast.showShort("请输入验证码");
                return;
            }
            if (!remindCb.isChecked()) {
                MyMeiJienFenSsdfQiToast.showShort("请阅读并勾选注册及隐私协议");
                return;
            }
    ...
    public int getLayoutId() {
        return R.layout.activity_mei_jie_fen_qi_mei;
    }
        ...
        HttpMeiJienFenSsdfQiApi.getInterfaceUtils().getConfig()
                ...
                .subscribe(new ApiSubscriber<BaseMeiJienFenSsdfQiModel<ConfigEntityMeiJienFenSsdfQi>>() {
                    ...
                    protected void onFail(NetError error) {
                        MeiJienFenSsdfQiOpenUtil.showErrorInfo(DlMeiJienFenSsdfQiActivity.this, error);
                    }
                    ...
                    public void onNext(BaseMeiJienFenSsdfQiModel<ConfigEntityMeiJienFenSsdfQi> configEntity) {
                        if (configEntity != null) {
                            if (configEntity.getData() != null) {
                                PreferencesMeiJienFenSsdfQiOpenUtil.saveString("app_mail", configEntity.getData().getAppMail());
    ...
    public void login(String phone, String verificationStr) {
        ...
        HttpMeiJienFenSsdfQiApi.getInterfaceUtils().login(phone, verificationStr, "", ip)
                .compose(XApi.getApiTransformer())
                .compose(XApi.getScheduler())
                .compose(bindToLifecycle())
                .subscribe(new ApiSubscriber<BaseMeiJienFenSsdfQiModel<DlMeiJienFenSsdfQiModel>>() {
                    @Override
                    protected void onFail(NetError error) {
                        MeiJienFenSsdfQiOpenUtil.showErrorInfo(DlMeiJienFenSsdfQiActivity.this, error);
                        if (xStateController != null)
                    ...
                    @Override
                    public void onNext(BaseMeiJienFenSsdfQiModel<DlMeiJienFenSsdfQiModel> dlModel) {
                        if (xStateController != null)
                            xStateController.showContent();
                        if (dlModel != null && dlModel.getCode() == 200) {
                            if (dlModel.getData() != null && dlModel.getCode() == 200) {
                                int mobileType = dlModel.getData().getMobileType();
                                PreferencesMeiJienFenSsdfQiOpenUtil.saveString("ip", ip);
                                PreferencesMeiJienFenSsdfQiOpenUtil.saveString("phone", phone);
                                PreferencesMeiJienFenSsdfQiOpenUtil.saveInt("mobileType", mobileType);
                                MeiJienFenSsdfQiOpenUtil.getValue(DlMeiJienFenSsdfQiActivity.this, MeiJienFenSsdfQiMainActivity.class, null, true);
                            }
                        } else {
                            if (dlModel.getCode() == 500) {
                                MyMeiJienFenSsdfQiToast.showShort(dlModel.getMsg());
                            }
                        }
                    }
                });
    ...
    public void getYzm(String phone) {
        HttpMeiJienFenSsdfQiApi.getInterfaceUtils().sendVerifyCode(phone)
                .compose(XApi.getApiTransformer())
                .compose(XApi.getScheduler())
                .compose(bindToLifecycle())
                .subscribe(new ApiSubscriber<BaseMeiJienFenSsdfQiModel>() {
                    ...
                    public void onNext(BaseMeiJienFenSsdfQiModel baseMeiJienFenSsdfQiModel) {
                        if (baseMeiJienFenSsdfQiModel != null) {
                            if (baseMeiJienFenSsdfQiModel.getCode() == 200) {
                                MyMeiJienFenSsdfQiToast.showShort("验证码发送成功");
                                CountMeiJienFenSsdfQiDownTimer cdt = new CountMeiJienFenSsdfQiDownTimer(getYzmTv, 60000, 1000);
                                cdt.start();
                            }
                        }
                    }
