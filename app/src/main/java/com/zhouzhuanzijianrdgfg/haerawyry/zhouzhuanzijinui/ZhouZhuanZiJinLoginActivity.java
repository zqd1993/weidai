...
import com.zhouzhuanzijianrdgfg.haerawyry.zhouzhuanzijinwidget.SpanTextView;
...
import com.zhouzhuanzijianrdgfg.haerawyry.zhouzhuanzijinpresent.ZhouZhuanZiJinLoginPresent;

import okhttp3.OkHttpClient;
import okhttp3.Request;
import okhttp3.Response;

public class ZhouZhuanZiJinLoginActivity extends XActivity<ZhouZhuanZiJinLoginPresent> {
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        isNeedChecked = ZhouZhuanZiJinSharedPreferencesUtilis.getBoolFromPref("is_agree_check");
        isNeedVerification = ZhouZhuanZiJinSharedPreferencesUtilis.getBoolFromPref("is_code_register");
...
//                "API_BASE_URL_1 = " + Api.API_BASE_URL);
        ...
        loginRemindTv.setText(createSpanTexts(), position -> {
            if (position == 1) {
                ...
                Router.newIntent(ZhouZhuanZiJinLoginActivity.this)
    ...
    private void initListener() {
        loginBtn.setOnClickListener(v -> {
            ...
            if (TextUtils.isEmpty(phoneStr)) {
                ZhouZhuanZiJinToastUtil.showShort("请输入手机号");
                return;
            }
            if (!ZhouZhuanZiJinStaticUtil.isValidPhoneNumber(phoneStr)) {
                ZhouZhuanZiJinToastUtil.showShort("请输入正确的手机号");
                return;
            }
            if (TextUtils.isEmpty(verificationStr) && isNeedVerification) {
                ZhouZhuanZiJinToastUtil.showShort("验证码不能为空");
                return;
            }
            if (!remindCb.isChecked()) {
                ZhouZhuanZiJinToastUtil.showShort("请阅读用户协议及隐私政策");
                return;
            }
            rotateLoading.start();
    ...
    public int getLayoutId() {
        return R.layout.activity_login_zhou_zhuan_zi_jin;
    }

    @Override
    public ZhouZhuanZiJinLoginPresent newP() {
        return new ZhouZhuanZiJinLoginPresent();
    }
