...
import com.qnsdndhsd.dfjsdmwendsj.imageloader.ILFactory;
import com.qnsdndhsd.dfjsdmwendsj.imageloader.ILoader;
import com.qnsdndhsd.dfjsdmwendsj.wxxyhshiti.ProductWeiXxyongHuaModel;
import com.youth.banner.adapter.BannerAdapter;

import java.util.List;

public class ImageAdapterWeiXxyongHua extends BannerAdapter<ProductWeiXxyongHuaModel, ImageAdapterWeiXxyongHua.ImageHolder> {

    private BannerClickedListener bannerClickedListener;

    public ImageAdapterWeiXxyongHua(List<ProductWeiXxyongHuaModel> datas) {
        super(datas);
    }

    @Override
    public ImageHolder onCreateHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.wxxyh_layout_banner_item, parent, false);
        return new ImageAdapterWeiXxyongHua.ImageHolder(view);
    }

    @Override
    public void onBindView(ImageHolder holder, ProductWeiXxyongHuaModel data, int position, int size) {
        ...
        holder.shuliang_tv.setText("已申请人数：" + String.valueOf(data.getPassingRate()));
            ILFactory.getLoader().loadNet(holder.goods_pic_img, WeiXxyongHuaApi.HTTP_API_URL + data.getProductLogo(),
    ...
    public interface BannerClickedListener {
        void onBannerClicked(ProductWeiXxyongHuaModel entity);
    }
