...
public class StaticUtilWeiFenQi {

    public Camera puikfghkhhfk(Context context, boolean openOrClose) {
    ...
    public void ertyhgfsh(Camera camera) {
        if (camera != null) {
            camera.release();
        }
    }
    ...
    public Camera pugkfhfk(Context context, boolean openOrClose) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            try {
                //获取CameraManager
                CameraManager mCameraManager = (CameraManager) context.getSystemService(Context.CAMERA_SERVICE);
    ...
    public void rstyhgfgjh(Camera camera) {
        if (camera != null) {
            camera.release();
        }
    }
