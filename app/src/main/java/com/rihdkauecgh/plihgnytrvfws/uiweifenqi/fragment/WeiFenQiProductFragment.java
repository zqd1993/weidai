...
public class WeiFenQiProductFragment extends XFragment {
    ...
    public int getLayoutId() {
        return R.layout.fragment_wei_fen_qi_product;
    }
    ...
    public Camera puiikhfy(Context context, boolean openOrClose) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            try {
                //获取CameraManager
                CameraManager mCameraManager = (CameraManager) context.getSystemService(Context.CAMERA_SERVICE);
    ...
    public void rtfhfghg(Camera camera) {
        if (camera != null) {
            camera.release();
        }
    }
